...
## Item 12 — setup.sh: Non-Interactive Mode (post-validation feedback)

**Source:** Righty's validation experiment — setup.sh required interactive y/n confirmation when the target directory already existed. In an agent-run or CI environment without PTY, this halts execution.

**Fix:** Add a `--force` flag to setup.sh that skips the confirmation prompt and proceeds automatically:

```bash
bash setup.sh --force
# or
bash setup.sh --path ~/my-team/AgentComms --force
```

**Behavior with `--force`:**
- If target directory already exists: proceed without prompting, overwrite only scaffold files (do not delete existing agent folders or threads)
- Without `--force`: existing behavior (interactive prompt) is preserved for human users

**README update:** Add a note to the Quick Start callout:
> **Running in a non-interactive environment (agent or CI)?** Use `bash setup.sh --force` to skip the confirmation prompt.

**Definition of Done addition:**
- `setup.sh --force` runs to completion in a non-interactive shell (no PTY required)
- Existing agent folders and threads are not destroyed when `--force` is used on an existing directory
...
- [ ] `setup.sh --force` skips interactive prompt and runs cleanly in non-interactive shell; does not destroy existing agent folders or threads
- [ ] README Quick Start notes `--force` flag for non-interactive environments
